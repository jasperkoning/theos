...
objects = $(foreach file, $(files), $(BUILD)/$(patsubst %.mm,%.o,$(patsubst %.xm,%.o,$(patsubst %.swift,%.o,$(file:.m=.o)))))
...
complile_print = @echo -e "\e[0;31m==>$(white) compiling $(patsubst %.xm,%.mm,$<) \e[0m"
link_print = @echo -e "\e[0;33m==>$(white) linking $(patsubst %.xm,%.mm,$<) \e[0m"
sign_print = @echo -e "\e[0;32m==>$(white) signing\e[0m"

$(BINARYDIR)/$(name): $(mainlink) $(library) Makefile
	@mkdir -p $(BINARYDIR)
	$(link_print)
ifneq ($(findstring .swift,$(files)),)
	swiftc -sdk $(JK_SDK) $(mainlink) -o $@
else
...
$(BUILD)/%.o: %.swift
	swiftc -c -sdk $(JK_SDK) $(files) -o $@

$(BUILD)/%.o: %.xm $(headers)
	@$(shell if [ $< -nt $(BUILD)/$(patsubst %.xm,%.mm,$<) ]; then echo -e "\e[0;36m==>$(white) logos\e[0m" > `tty`; if ! $(THEOS)/bin/logos.pl $< > $(BUILD)/$(patsubst %.xm,%.mm,$<); then echo jkLOGOSFAILED; rm $(BUILD)/$(patsubst %.xm,%.mm,$<); fi; else echo no logos > `tty`;  fi)
